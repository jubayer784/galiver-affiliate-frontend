...
const cards = [
  ...
  { key: 'pending', label: 'Total Pending', icon: 'pending', color: '#3978b8' },
  { key: 'canceled', label: 'Total Cancel', icon: 'cancel', color: '#c94b4b' },
  { key: 'returned', label: 'Total Return', icon: 'assignment_return', color: '#9a6915' },
  { key: 'earning', label: 'Total Earning', icon: 'payments', color: '#7a4aa8', money: true },
];
...
export default function OrderStats() {
  ...
  return (
    <section className="order-stats" aria-label="Order summary">
      ...
      <div className="order-stats-grid">{cards.map(card => <article className="order-stat-card" key={card.key}><span className="order-stat-icon" style={{ background: card.color }}><span className="material-symbols">{card.icon}</span></span><span><span className="order-stat-label">{card.label}</span><strong className="order-stat-value">{card.money ? `৳ ${Number(stats[card.key] || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : Number(stats[card.key] || 0).toLocaleString('en-US')}</strong></span></article>)}</div>
    </section>
  );
}
