...
const initialCounts = { total: 0, pending: 0, delivered: 0, canceled: 0, returned: 0 };
const zeroEarnStatuses = ['canceled', 'returned', 'partial'];
const orderEarn = order => {
  if (typeof order.earn === 'number') return order.earn;
  const value = (order.orderProduct || []).reduce((sum, item) => sum + Number(item.price || 0) * Number(item.quantity || 0), 0);
  return zeroEarnStatuses.includes(order.status) ? 0 : value * 0.1;
};
const earnTone = status => (status === 'delivered' ? 'green' : zeroEarnStatuses.includes(status) ? 'red' : 'amber');
const dateTime = value => ({
  date: new Date(value).toLocaleDateString('en-GB'),
  time: new Date(value).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true }),
});
...
export default function Orders() {
  ...
  return <main className="orders-page shell">
    <style>{`.orders-page{padding-bottom:120px}.orders-heading{margin:20px 0 18px;color:#17231d;font:700 30px Arial,sans-serif}.date-presets{display:grid;grid-template-columns:repeat(6,1fr);gap:8px;margin-bottom:10px}.date-preset{min-height:34px;padding:7px 8px;border:1px solid #dfe5dc;border-radius:6px;background:#fff;color:#526159;font:700 11px Arial,sans-serif;cursor:pointer}.date-preset:hover,.date-preset:focus-visible,.date-preset.active{border-color:#e35d38;background:#e35d38;color:#fff;outline:0}.order-date-filter{display:grid;grid-template-columns:1fr 1fr;align-items:end;gap:10px;margin-bottom:16px;padding:14px;border:1px solid #dfe5dc;border-radius:8px;background:#fff}.order-date-filter label{display:grid;gap:6px;color:#526159;font:700 11px Arial,sans-serif}.order-date-filter input{height:38px;padding:0 9px;border:1px solid #dfe5dc;border-radius:6px;background:#fff;color:#17231d;font:13px Arial,sans-serif}.orders-summary{display:grid;grid-template-columns:repeat(5,1fr);gap:12px;margin-bottom:26px}.orders-card{padding:16px;border:1px solid #dfe5dc;border-radius:8px;background:#fff}.orders-card-label{display:block;color:#6b776f;font:600 12px Arial,sans-serif}.orders-card-value{display:block;margin-top:9px;color:#17231d;font:700 26px Arial,sans-serif}.orders-card:nth-child(2) .orders-card-value{color:#c78319}.orders-card:nth-child(3) .orders-card-value{color:#20815a}.orders-card:nth-child(4) .orders-card-value{color:#c24848}.orders-card:nth-child(5) .orders-card-value{color:#c24848}.orders-list{overflow:hidden;border:1px solid #dfe5dc;border-radius:8px;background:#fff}.orders-list-heading{display:flex;justify-content:space-between;padding:16px;border-bottom:1px solid #e9ede7;color:#17231d;font:700 15px Arial,sans-serif}.order-row{display:grid;grid-template-columns:1.1fr .8fr 1.1fr .9fr .9fr;gap:12px;align-items:center;padding:15px 16px;border-bottom:1px solid #eef1ed;color:#526159;font:13px Arial,sans-serif}.order-row:last-child{border-bottom:0}.order-invoice{color:#17231d;font-weight:700}.order-date small{display:block;margin-top:2px;color:#94a09a;font-size:11px}.order-earn{font-weight:700}.order-earn.green{color:#20815a}.order-earn.red{color:#c24848}.order-earn.amber{color:#c78319}.order-status{text-transform:capitalize;color:#c78319;font-weight:700}.order-status.delivered{color:#20815a}.order-status.canceled,.order-status.returned,.order-status.partial{color:#c24848}.orders-empty,.orders-error{padding:24px;color:#6b776f;font:13px Arial,sans-serif}.orders-error{color:#b33b3b;background:#fff0ef}@media(max-width:700px){.orders-heading{font-size:25px}.date-presets{grid-template-columns:repeat(3,1fr)}.order-date-filter{grid-template-columns:1fr 1fr}.orders-summary{grid-template-columns:repeat(2,1fr);gap:10px}.orders-card{padding:14px}.orders-card-value{font-size:24px}.order-row{grid-template-columns:1fr 1fr;gap:8px 10px}.order-row span:nth-child(n+3){font-size:12px}}@media(max-width:430px){.date-presets{gap:6px}.date-preset{padding:7px 4px;font-size:10px}.order-date-filter{grid-template-columns:1fr 1fr}.orders-list-heading{font-size:14px}}`}</style>
    ...
    <section className="orders-summary" aria-label="Order summary">
      ...
      <article className="orders-card"><span className="orders-card-label">Total Delivered</span><strong className="orders-card-value">{counts.delivered}</strong></article>
    ...
    <section className="orders-list"><div className="orders-list-heading"><span>Order History</span><span>{loading ? 'Loading...' : `${orders.length} orders · ৳ ${orders.reduce((sum, order) => sum + (order.status === 'delivered' ? orderEarn(order) : 0), 0).toFixed(2)} earned`}</span></div>{!loading && !orders.length && <p className="orders-empty">এই date range-এ কোনো order পাওয়া যায়নি।</p>}{orders.map(order => { const when = dateTime(order.createdAt); return <div className="order-row" key={order._id}><span className="order-invoice">#{order.invoice || order._id.slice(-8)}</span><span>৳ {Number(order.total || 0).toLocaleString('en-US')}</span><span className="order-date">{when.date}<small>{when.time}</small></span><span className={`order-earn ${earnTone(order.status)}`}>৳ {orderEarn(order).toFixed(2)}</span><span className={`order-status ${order.status}`}>{order.status}</span></div>; })}</section>
  </main>;
}
