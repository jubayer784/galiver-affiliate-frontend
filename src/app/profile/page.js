...
export default function ProfilePage() {
  ...
  async function addPaymentMethod(event) {
    ...
    try {
      const token = localStorage.getItem('affiliateToken');
      const response = await fetch(`${apiUrl}/affiliate/payment-methods`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token || ''}` }, body: JSON.stringify({ method }) });
