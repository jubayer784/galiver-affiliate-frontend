...
export default function Login(){
  ...
  async function submit(event){event.preventDefault();setLoading(true);setError('');try{const response=await fetch(`${apiUrl}/affiliate/login`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(form)});const data=await response.json();if(data.status==='pending'){router.push('/pending');return}if(!response.ok)throw new Error(data.message||'Login failed');localStorage.setItem('affiliateToken',data.token);localStorage.setItem('affiliateProfile',JSON.stringify(data.affiliate));router.push('/dashboard')}catch(error){setError(error.message)}finally{setLoading(false)}}
